// ════════════════════════════════════════════════════════════
// db.js v4 — Backend en Supabase (gratis para siempre)
// Misma API pública que v3 — solo cambia la capa de red.
//
// PASOS PARA ACTIVAR:
//  1. Crea proyecto en supabase.com (gratis, sin tarjeta)
//  2. Ejecuta supabase-schema.sql en Dashboard → SQL Editor
//  3. Copia tu Project URL y anon key aquí abajo
// ════════════════════════════════════════════════════════════
(function(){
  "use strict";
  ...
  var SUPABASE_URL = "https://afzhkcuyocklhyiivdal.supabase.co";
  var SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFmemhrY3V5b2NrbGh5aWl2ZGFsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODUzODU4NTIsImV4cCI6MjEwMDk2MTg1Mn0.HdyaeqCkMqpFxQALGC1PGIdXgfLVJ90gEZgcsMQtoLU";

  // URL de la función serverless para escaneo IA de comida.
  // Déjalo vacío por ahora — funciona todo lo demás sin esto.
  var VISION_URL = "";
  ...
  // ── HELPERS DE RED ─────────────────────────────────────────
  // Convierte { alumno_id:"x", fecha_gte:"2024-01-01" }
  // en ?select=*&alumno_id=eq.x&fecha=gte.2024-01-01
  function qs(params){
    var parts = ["select=*"];
    Object.keys(params||{}).forEach(function(k){
      var v = params[k];
      if(v === undefined || v === null) return;
      var m = k.match(/^(.+)_(gte|lte|gt|lt|neq)$/);
      if(m) parts.push(m[1]+"="+m[2]+"."+v);
  ...
  function apiGet(table, params){
    return fetch(BASE+"/"+table+qs(params), { headers:H })
      .then(function(r){
  ...
  // Upsert: inserta o actualiza según la clave primaria de la tabla
  ...
  function apiPatch(table, params, body){
    var parts=[];
    Object.keys(params||{}).forEach(function(k){ if(params[k]!=null) parts.push(k+"=eq."+params[k]); });
    return fetch(BASE+"/"+table+(parts.length?"?"+parts.join("&"):""), {
      method:"PATCH",
  ...
  function apiUploadFile(file, prefix){
    var path = (prefix||"uploads")+"/"+Date.now()+"_"+file.name.replace(/[^a-zA-Z0-9._-]/g,"_");
    return fetch(STORAGE+"/fotos/"+path, {
      method:"POST",
      headers:{
        "apikey": SUPABASE_KEY,
        "Authorization":"Bearer "+SUPABASE_KEY,
  ...
  function apiWrite(fn){
    fn().then(function(r){ if(r&&r.error) console.warn("[db:write]",r.error); })
        .catch(function(e){ console.warn("[db:write]",e.message); });
  }
  ...
  // ── CACHÉ EN MEMORIA ────────────────────────────────────────
  var C = {
    alumnos:[], rutinas:[], planes:[], ejercicios:[], vinculos:[],
    gym_info:null, _aid:null,
    registros:[], pesos:[], medidas:[], medallas:[], notas:[],
    habitos:[], habito_checks:{}, nutricion:{}, progreso:{},
    fitscore:{}, food_scans:{}, objetivos:[], fotos:[]
  };
  ...
  // ════════════════════════════════════════════════════════════
  // window.db — API pública (idéntica a v3)
  // ════════════════════════════════════════════════════════════
  window.db = {
    ...
    savePeso: function(alumnoId, peso){
      ...
      var idx=C.pesos.findIndex(function(p){ return p.alumno_id===alumnoId&&p.fecha===peso.fecha; });
      if(idx>=0) C.pesos[idx]=peso; else C.pesos.push(peso);
    ...
    saveNutricion: function(alumnoId, fecha, datos){
      ...
      apiWrite(function(){
        return apiPost("nutricion_diaria",{
          ...
          opciones:datos.opciones||{}, comidos:datos.comidos||{},
          agua:datos.agua||0, alimentos:datos.alimentos||[],
          extras:datos.extras||[], suplementos:datos.suplementos||{}
        });
      });
    },
    getNutricionExtras: function(alumnoId, fecha){ return (C.nutricion[fecha]&&C.nutricion[fecha].extras)||[]; },
    ...
    checkMedallas: function(alumnoId){
      var self=this, registros=this.getRegistros(alumnoId), nuevas=[];
    ...
    saveNota: function(alumnoId, nota){
      ...
      if(!nota.id) nota.id=generarId("nota");
      C.notas.push(nota);
    ...
    migrateGymIfNeeded: function(){
      var self=this;
      var g=C.gym_info;
      if(!g || g.nombre !== "Partner Gym EC"){
        self.saveGymInfo({
          activo:true,
          nombre:"Partner Gym EC",tagline:"Entrena diferente. Crece en comunidad.",
          logo_url:"",sede:"Sede El Tejar",
          direccion:"El Tejar, Quito, Ecuador",
          maps_url:"https://maps.google.com/?q=Partner+Gym+EC+El+Tejar+Quito",
          telefono:"+593999999999",whatsapp:"+593999999999",instagram:"partnergymec",
          horarios_atencion:[
          ...
          servicios:[
            {icono:"pesas",nombre:"Sala de pesas"},{icono:"spinning",nombre:"Spinning / Indoor Cycling"},
            {icono:"yoga",nombre:"Yoga y Pilates"},{icono:"cardio",nombre:"Bailoterapia"},
            {icono:"cardio",nombre:"HYROX"},{icono:"cardio",nombre:"Kick Boxing"},{icono:"vestuarios",nombre:"Vestuarios"}
          ],
          galeria:[],videos_educativos:[],videos_recetas:[],
          anuncios:[{id:"anuncio_001",titulo:"¡Bienvenido a Partner Gym EC!",texto:"Entrenamos diferente. Cada clase, cada rep, te acerca a tu mejor versión. 💪",fecha:"2026-08-01",destacado:true}],
          normas:["Trae siempre tu toalla personal.","Limpia el equipo después de usarlo.","Respeta los turnos en las máquinas.","No se permite comida en la sala de pesas.","Reserva tu clase con anticipación."],
          cupones:[],promociones:[],
    ...
    // ── VISIÓN IA (escaneo de comida) ─────────────────────────
    // Requiere una función serverless con la OpenAI API key.
    // Configura VISION_URL arriba cuando tengas el endpoint.
    escanearComidaIA: function(file){
      if(!VISION_URL){
        return Promise.reject(new Error("El escaneo IA no está configurado. Contacta al administrador."));
      }
      var fd=new FormData();
      fd.append("file",file);
      return fetch(VISION_URL,{method:"POST",body:fd})
        .then(function(r){
    ...
    // ── FOOD SCANS ───────────────────────────────────────────
    getFoodScans: function(alumnoId, fecha){ return (C.food_scans[fecha||fechaHoy()]||[]).slice(); },
    saveFoodScan: function(alumnoId, scan){
      ...
      if(!scan.id) scan.id=generarId("scan");
      apiWrite(function(){ return apiPost("food_scans",{id:scan.id,alumno_id:alumnoId,fecha:fecha,scan_data:scan}); });
    },
    ...
    getProgresoDiario: function(alumnoId, fecha){
      return Object.assign({pasos:0,agua_ml:0,sueno_h:0,calorias_activas:0},C.progreso[fecha||fechaHoy()]||{});
    },
    ...
    vincularAlumnos: function(alumnoId1, alumnoId2){
      var yaExiste=C.vinculos.find(function(v){ return (v.alumno1===alumnoId1&&v.alumno2===alumnoId2)||(v.alumno1===alumnoId2&&v.alumno2===alumnoId1); });
    ...
    getVinculoDe: function(alumnoId){ return C.vinculos.find(function(v){ return v.alumno1===alumnoId||v.alumno2===alumnoId; })||null; },
    ...
    seedDemo: function(){
      ...
      var alumno={
        id:"a_demo1",codigo:"1111",nombre:"Santiago",apellido:"Guamán",edad:19,genero:"masculino",
        objetivo:"perdida_grasa",nivel:"principiante",peso_inicial:65,peso_actual:65,altura:167,
