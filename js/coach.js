...
(function(){
  ...
  var SECCIONES = ["dashboard","alumnos","rutinas","alimentacion","ejercicios","mensajes","gym"];
  ...
  function renderAlumnoDetalle(id){
    ...
    var tabs = ["resumen","rutina","alimentacion","habitos","progreso","notas"];
    ...
    function renderTab(){
      ...
      if(tabActivo === "resumen"){
      ...
      } else if(tabActivo === "habitos"){
        var habitos = window.db.getHabitos(a.id);
        html = "<div class='coach-card'>" +
          habitos.map(function(h){ return "<div class='ejercicio-builder-row'><span>" + h.nombre + " — " + (h.frecuencia||"diario").replace("_"," ") + "</span>" +
            "<button class='btn-quitar-habito' data-id='" + h.id + "' style='background:none;border:none;color:#FF453A;cursor:pointer;'>✕</button></div>"; }).join("") +
          (habitos.length===0 ? "<p style='color:#999;'>Sin hábitos asignados.</p>" : "") +
          "<button class='btn-coach secondary' id='btn-add-habito-coach' style='margin-top:12px;'>+ Asignar hábito</button></div>";
        ...
        var fotosCompartidas = window.db.getFotos(a.id).filter(function(f){ return f.compartida; });
        ...
        html += "<div class='coach-card'><h3 style='margin-bottom:12px;'>Fotos de progreso compartidas</h3>" +
          (fotosCompartidas.length === 0
            ? "<p style='color:#999;font-size:.85rem;'>El alumno no ha compartido fotos contigo todavía. Por defecto todas sus fotos son privadas.</p>"
            : "<div style='display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:10px;'>" +
              fotosCompartidas.map(function(f){ return "<div><img src='" + f.dataUrl + "' style='width:100%;border-radius:10px;'><p style='font-size:.7rem;color:#999;margin-top:4px;'>" + f.fecha + "</p></div>"; }).join("") + "</div>") +
          "</div>";
      ...
      if(tabActivo === "habitos"){
        document.querySelectorAll(".btn-quitar-habito").forEach(function(btn){
          btn.addEventListener("click", function(){ window.db.deleteHabito(a.id, this.getAttribute("data-id")); renderTab(); });
        });
        $("#btn-add-habito-coach").addEventListener("click", function(){
          coachModal("Asignar hábito", "<div class='coach-form'>" +
            "<label>Nombre del hábito</label><input id='ch-nombre' placeholder='Ej: Tomar 2 litros de agua'>" +
            "<label>Frecuencia</label><select id='ch-frecuencia'><option value='diario'>Todos los días</option><option value='dias_entreno'>Solo días de entreno</option><option value='fines_semana'>Fines de semana</option></select>" +
            "<button class='btn-coach' id='ch-guardar' style='margin-top:14px;'>Guardar hábito</button></div>", function(){
            $("#ch-guardar").addEventListener("click", function(){
              var nombre = $("#ch-nombre").value.trim();
              if(!nombre) return;
              window.db.saveHabito(a.id, { id: window.db.generarId("h"), nombre: nombre, icono:"default", frecuencia: $("#ch-frecuencia").value, creado: window.db.fechaHoy() });
              window.cerrarCoachModal();
              renderTab();
            });
          });
